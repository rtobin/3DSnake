(function () {
  ...
  Map.prototype.init = function ( callback ) {
    ...
    this.rewardImage.src = 'images/apple.png';
  ...
  Map.prototype.updateRewardOpacity = function ( locId ) {
    // fade reward appearance depending on distance from head node
    var fadeness = this.locations[ locId ].position
                  .distanceTo(this.reward.position) * Map.OPACITY_SCALING;
    this.reward.element.style.opacity = 1 - fadeness;
  };
  ...
  Map.prototype.makeLocationVacant = function ( locId ) {
    var sphere = this.locations[ locId ];
    if ( ! sphere.poop ) {
      sphere.element.style.opacity = "0.1";
      sphere.isOccupied = false;
      $(sphere.element).css( 'background', 'transparent' );
    } else {
      // debugger
    }
