(function () {
  ...
  var Game = CUBOA.Game = function ( options ) {
    ...
    this.numHumans = options.numHumans;
    this.numComputers = options.numComputers;
    this.numPlayers = this.numHumans + this.numComputers;
    this.players = [];

    for ( var i = 0; i < this.numHumans; i ++ ) {
      this.players.push( new CUBOA.HumanPlayer( this.map, i ) );
    }

    for ( var i = this.numHumans; i < this.numHumans + this.numComputers; i ++ ) {
      this.players.push( new CUBOA.AIPlayer( this.map, i, CUBOA.Util.AStar ) );
    }
