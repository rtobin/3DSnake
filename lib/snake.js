(function () {
  ...
  var Snake = CUBOA.Snake = function ( map, id ) {
    ...
    this.isAlive = true;
    this.score = 0;
    this.growing = 0; // integer indicating nummber of segments to grow
  };
    ...
    if ( this.isAlive ) {
      ...
      if ( this.length > 0 && this.map.locations[ locId ].isOccupied) {
        ...
        if ( locId === this.map.rewardLocId ) {
          ...
          if ( CUBOA.Game.POOP ) {
            this.map.makeLocationBricked( locId, this.id );
          }
        }
        this.addToFront( locId );
  ...
  Snake.prototype.getReward = function () {
    ...
    $("#score-pts").val( this.score.toString() );

    this.map.placeReward();
  };

  Snake.prototype.updateNextMoveVecDelta = function () {
    var sideNormalVec = new THREE.Vector3();
    sideNormalVec.copy(CUBOA.Game.UNIT_VECS[this.currentSide]);
    switch ( this.turningDir ) {
      case 'left':
        this.nextMoveVecDelta.crossVectors( sideNormalVec , this.nextMoveVecDelta );
        break;
      case 'right':
        this.nextMoveVecDelta.crossVectors ( this.nextMoveVecDelta , sideNormalVec );
        break
      case 'pause':
        if (this.paused) {
  ...
  Snake.prototype.getNextLocation = function () {
    this.updateNextMoveVecDelta();
